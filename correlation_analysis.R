...
for(x in ricker_stocks$stock_name){
  ...
  # find max SpBio
  max_sb <- max(stock[,3], na.rm = TRUE)
  
  # find S-R pairs less than 50% of max & remove from dataset
  for(y in 1:nrow(stock)){
    if(!is.na(stock[y,3]) && stock[y,3] < 0.5*max_sb){
      stock<- stock[-c(y),]
    }
  }
  ...
  ricker_stocks[row, 10] <- corrr$estimate
  ricker_stocks[row, 11] <- corrr$p.value
}
...
for(x in bevholt_stocks$stock_name){
  ...
  bevholt_stocks[row,10] <- zero_lag
  ...
  for(x in 1:10){
    if(stock_ccf_df[x,2] <= zero_lag){
      bevholt_stocks[row, 11] <- 1
    }else{
      bevholt_stocks[row, 11] <- 0
    }
  }
}
